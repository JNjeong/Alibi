...
function ChatPanel({
  ...
  useEffect(() => {
    const frameId = window.requestAnimationFrame(() => {
      const chatList = scrollRef.current
  ...
  }, [messages.length, activeTab, game.stage, game.currentRound])
    ...
    <section className="chat-panel">
      <div className="section-title-row chat-title">
        <div>
          ...
          <span className="online-count">
            <i />
            {connected ? `${game.players.length}명 참가 · 연결됨` : "재연결 중"}
          </span>
        </div>
        <span>실시간</span>
      </div>
      ...
      <form className="chat-form" onSubmit={onSubmit}>
        ...
        <input
          ...
          disabled={!connected}
        />
        <button type="submit" aria-label="메시지 전송" disabled={!connected}>
          ↑
        </button>
      </form>
    </section>
