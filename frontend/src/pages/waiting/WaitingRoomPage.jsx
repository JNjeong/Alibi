...
function WaitingRoomPage() {
  ...
  // 친구 요청
  const [friendStatusByUserId, setFriendStatusByUserId] = useState({})
  ...
  const fetchFriendStatuses = async () => {
    try {
      ...
      // 나에게 온 요청
      for (const request of receivedData.friendRequests || []) {
        const requester = request.requester

        if (requester?._id) {
          statusMap[String(requester._id)] = "received"
        }
      }

      // 내가 보낸 요청
      for (const request of sentData.sentRequests || []) {
        const receiver = request.receiver

        if (receiver?._id) {
          statusMap[String(receiver._id)] = "sent"
        }
      }
      ...
      for (const friendship of friendsData.friends || []) {
        ...
        const friend =
          String(requester?._id) === String(currentUserId)
            ? receiver
            : requester

        if (friend?._id) {
          statusMap[String(friend._id)] = "friend"
        }
      }

      if (!cancelled) {
        setFriendStatusByUserId(statusMap)
      }
    } catch (error) {
      console.error("대기실 친구 상태 조회 실패:", error)
    }
  }
...
const handleSendFriendRequest = async (participant) => {
  if (!participant?._id || !participant?.username) {
    return
  }

  try {
    setSendingFriendId(String(participant._id))

    // 현재 친구 API는 MongoDB _id가 아니라 username을 받음
    const data = await sendFriendRequest(participant.username)

    setFriendStatusByUserId((previous) => ({
      ...previous,
      [String(participant._id)]: "sent",
    }))
