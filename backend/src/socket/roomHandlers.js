...
import { getGamePlayerLimits } from "../config/gameConfig.js"
import { makePublicGameError, startGame, skipGameStage } from "../services/game_service.js"
...
const buildRoomPayload = (room, onlineIds, readySet) => {
  ...
  return {
    ...
    canStart:
      hostOnline &&
      participants.length >= getGamePlayerLimits().minPlayers &&
      participants.length <= getGamePlayerLimits().maxPlayers &&
      allNonHostReady,
  }
}
...
export const formatRoomMessage = (messageDoc) => ({
...
export const loadRoomChatMessages = async (roomId) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("room:start", async ({ roomId }, callback) => {
      try {
        ...
        await io.gameTimer?.schedule(result.gameId)
        ...
        const payload = makePublicGameError(
          error,
          "게임 시작 중 오류가 발생했습니다."
        )
    ...
    // 시간 스킵 작동
    socket.on("game:skip-stage", async ({ gameId }) => {
      try {
        ...
        socket.emit("game:stage:skipped", result)
      } catch (error) {
        console.error("게임 시간 스킵 오류:", error)

        socket.emit("game:stage:skip-error", {
