...
const playerSnapshotSchema = new Schema(
  {
    ...
    // 5개 라운드에서 한 번씩 질문할 수 있도록 게임 전체 최대 5회입니다.
    questionCount: {
      type: Number,
      default: 0,
      min: 0,
...
const officialRecordSchema = new Schema(
  {
    ...
    // v5 호환 필드입니다. v6에서는 공식 진술/답변 timeout record를 만들지 않으므로
    // 신규 officialRecord는 모두 player이며, 기존 system_timeout 문서를 읽을 때만 의미가 있습니다.
    submissionSource: {
      type: String,
      enum: ["player", "system_timeout"],
      default: "player",
    },

    // lifecycle 상태와 모순 검증 상태를 분리합니다.
    // - statement/answer: submitted
    // - question: pending -> answered | timed_out
    // legacy 값은 기존 v5 문서를 읽을 수 있도록 당분간 허용하지만 v6 신규 기록에는 쓰지 않습니다.
    status: {
      type: String,
      enum: ["submitted", "pending", "answered", "timed_out", "verified", "contradiction"],
      default: "submitted",
    },

    // 질문 자체는 논리 주장이 아니므로 null입니다.
    // 공식 진술과 공식 답변만 unchecked -> verified | contradiction으로 갱신합니다.
    validationStatus: {
      type: String,
      enum: ["unchecked", "verified", "contradiction", null],
      default: null,
    },
...
const gameSchema = new Schema(
  {
    ...
    // playing은 진행 중, finished는 정상 종료, forced는 관리자 강제 종료입니다.
    status: {
      type: String,
      enum: ["playing", "finished", "forced"],
      default: "playing",
      index: true,
    },

    // phase는 기존 화면·관리자 코드와의 호환을 위한 큰 진행 구분입니다.
    phase: {
      type: String,
    ...
    // 실제 제출 가능 여부와 서버 타이머는 세부 stage를 기준으로 판단합니다.
    stage: {
      type: String,
      enum: [
        "statement",
    ...
    // 여러 타이머 또는 서버 인스턴스가 같은 만료 단계를 동시에 처리하지 않게 합니다.
    progressionLock: {
      type: Schema.Types.Mixed,
      default: null,
    },
    ...
    players: {
      ...
      validate: {
        validator: (players) => {
          ...
          return players.length >= minPlayers && players.length <= maxPlayers
        },
        message: () => {
          const { minPlayers, maxPlayers } = getGamePlayerLimits()
          return `ALIBI 게임 참가자는 ${minPlayers}~${maxPlayers}명이어야 합니다.`
        },
      },
    },
    ...
    // setGame()이 만든 정답·실제 타임라인·목격정보입니다.
    // 일반 조회에서는 숨기고 game_service.js만 판정과 자기 정보 조립에 사용합니다.
    secretData: {
      type: Schema.Types.Mixed,
      required: true,
      select: false,
